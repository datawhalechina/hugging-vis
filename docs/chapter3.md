...
流模型（Flow-based Models）是一类生成模型，通过可逆变换将简单分布（如高斯分布）映射到复杂数据分布。NICE是早期流模型，通过加性耦合层实现可逆变换，保持雅可比行列式为1，简化计算。RealNVP扩展了NICE，允许雅可比行列式不为1，提升了表达能力。Glow进一步改进，结合可逆1x1卷积和仿射耦合层，生成高质量图像。这些模型通过可逆变换和高效雅可比行列式计算，实现了精确的密度估计和样本生成。相比于这些传统的基于可逆映射的 Normalizing Flow，Flow Matching方法提供了一种新的视角，将生成建模问题表述为在连续时间下学习一个确定性流动（ODE），大大提升了生成效率和质量。
...
传统的生成模型有两大类：（1）Normalizing Flow 通过可逆变换直接计算似然，但对可逆结构设计要求高。（2）Diffusion 模型，基于随机微分方程（SDE），通过逐步去噪实现生成，训练稳定、效果好，但采样速度慢。

Flow Matching 提出了一种新的方法，试图融合两者优点：

* 不依赖逐层可逆结构（比 Flow 灵活）。
* 通过 ODE（常微分方程）而不是 SDE（随机微分方程）来描述生成过程（比 Diffusion 更高效）。

其核心思想是：给定起始分布（通常是高斯噪声）和目标数据分布，构造一条从噪声流向数据的连续轨迹。Flow Matching 不再依赖于逐层可逆结构和显式的雅可比行列式算，而是直接训练一个神经网络来近似速度场 $v_\theta(x_t, t)$，使其能够驱动样本沿着 ODE 轨迹演化。

$$
\frac{dx_t}{dt} = v_\theta(x_t, t), \quad x_0 \sim p_0, \quad x_1 \sim p_{\text{data}}
$$

其中，$x_0$：起始分布（一般是高斯噪声）；$x_1$：目标数据分布；$v_\theta(x_t, t)$：由神经网络参数化的速度场。

在训练过程中，Flow Matching 采用“噪声—数据直线插值”作为监督信号：对于任意一对噪声样本 $x_0$ 和真实样本 $x_1$，在时间 $t \in [0,1]$ 构造插值点 $x_t = (1-t)x_0 + t x_1$，其真实速度为 $x_1 - x_0$。模型通过拟合该速度场来学习从起始分布到目标分布的映射。生成阶段则通过数值求解学到的 ODE，从噪声确定性演化到数据分布，实现图像生成。所以其训练优化目标为：

$$
\min_\theta \; \mathbb{E}\Big[\| v_\theta(x_t, t) - (x_1 - x_0) \|^2 \Big]
$$

模型直接拟合从噪声到数据的“传输速度”，避免了 Diffusion 中复杂的 score function 学习。训练好 $v_\theta$ 后，生成时只需从高斯噪声采样 $x_0$。然后解 ODE：$\frac{dx_t}{dt} = v_\theta(x_t, t), \quad t:0 \to 1$。最后得到 $x_1$，即合成图像。数值解法常用 Runge-Kutta (RK4)、Euler 等，可以在很少步数内完成。

Flow Matching 在概念上连接了 **Diffusion 模型** 和 **Flow-based 模型**：它像 Diffusion 一样从噪声逐渐转化为数据，但过程是确定性的 ODE 而非随机的 SDE；它又像 Normalizing Flow 一样描述连续可逆的分布变换，但不需要设计逐层可逆的显式结构。凭借这一特性，Flow Matching 既保留了流模型的连续可逆建模优势，又简化了训练与采样流程，能够在较少步数下生成高质量样本。
